...
const cleanUp = async ()=>{
    ...
    const closeThings = async ()=>{
        const toSell =   await getRepository(ToSell)
    .createQueryBuilder('toSell')
    .orderBy('toSell.price', 'DESC').getMany()
    return await Promise.all(toSell.map(async ts=>{
    ...
    console.log('-----------------------------------------------------------')
    const fixThings = (coin1:string, coin2:string,  coin3:string) => Object.entries(balances).map( async ([coin0 ,b ])=>{
        if(bn(b).isZero()){
        ...
        if(bn(b).isGreaterThan('0') && coin0!==coin1 && coin0!==coin2 && coin0!==coin3){
            ...
            .where('(toSell.pairName = :pair0 or toSell.pairName = :pair1 or toSell.pairName = :pair3) AND toSell.filled = :filled', {
              pair0: coin0+coin2,
              pair1: coin0+coin1,
              pair3: coin0+coin3,
              filled: false,
        ...
        if(toSellPosition.length>0 ){
            ...
            console.log(`${ts.id} changed ${coin0} from ${howMuchToSell} by ${diff}   (balance:${b}), will be removed: `,bn(ts.left).isLessThan(0))
            ts.left = bn(ts.left).plus(diff).toFixed(8)
            ts.filled = false
            if(bn(ts.left).isLessThan(0))
            await ts.remove()
            else
            await ts.save()
        }
