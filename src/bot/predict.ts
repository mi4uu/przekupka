...
console.log(
  'balanceTransactionType,minChange,risk,badTransactionsCount,btc,usd,worthInUsdStartingPrice,worthInUsdEndPrice,profitTransDiff,profit,buyCount,sellCount,buyAvgCost,sellAvgCost,day0,day1,day3,day4,day5,day6,day7,day8,day9',
)
...
const runSimulation = (minChange: BigNumber, risk: BigNumber, balanceTransactionType: number) => {
  ...
  const BTCSTART = new BigNumber('1')
  const USDSTART = new BigNumber('10000')
  const volume = new BigNumber('0.001')
  const days: string[] = []
  ...
  const inputStream = Fs.createReadStream('./.kraken2m.csv', 'utf8')
  inputStream
    ...
    .on('data', function (row) {
      ...
      if (counter > 50000) {
        ...
        const valueStart = USD.plus(BTC.multipliedBy(startingPrice)).minus(
          USDSTART.plus(BTCSTART.multipliedBy(startingPrice)),
        )
        const valueStop = USD.plus(BTC.multipliedBy(endingPrice)).minus(
          USDSTART.plus(BTCSTART.multipliedBy(endingPrice)),
        )
    ...
    .on('end', function () {
      ...
      const transNo =
        transactions.b.length > transactions.s.length ? transactions.s.length - 2 : transactions.b.length - 2
      ...
      msg += `,${transactions.s
        .slice(0, transNo)
        .reduce((p, c) => c.plus(p), new BigNumber(0))
        .minus(transactions.b.slice(0, transNo).reduce((p, c) => c.plus(p), new BigNumber(0)))
        .toFixed(8)}`
      msg += `,${transactions.b.length},${transactions.s.length},${tbs.toFixed(8)},${tss.toFixed(8)}`
      for (const day of days) msg += `,${day}`
      console.log(msg)
    })
}
