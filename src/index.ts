...
if(args.length<3)
{
  console.log('npm start ['+chalk.blue('minChange')+'] ['+chalk.blue('minChangeTrend')+'] ['+chalk.blue('howMuchToTrade')+'] ['+chalk.blue('pair')+'] ['+chalk.blue('amount1')+'] ['+chalk.blue('amount2')+']' )
  console.log('* '+chalk.blue('minChange')+' - min change in price to consider buy/sell (ex. 0.5)')
  console.log('* '+chalk.blue('minChangeTrend')+' - dow much price has to change to make decision of buying/selling (ex. 0.2)')
...
let lowest = new BigNumber(2e+308)
...
const buyFromMarket = () => {
  userRef+=1
  let usdToSell = minSize.multipliedBy(howMuchToTrade).multipliedBy(price)
...
const sellFromMarket = () => {
  userRef+=1
  let btcToSell =  minSize.multipliedBy(howMuchToTrade)
...
const watch = async () => {
  ...
  console.log(chalk`RUN PARAMS: [minChange: {blue ${minChange}}] [minChangeTrend: {blue ${minChangeToConsiderChangeOfTrend}}]  [howMuchToTrade: {blue ${howMuchToTrade}}]  [pair: {blue ${pair}}]`)
  
  console.log(coin[1]+' was: '+chalk.blue.bold(usdBase.toFixed(pairDecimals))+'      '+coin[0]+' was: '+chalk.blue.bold(btcBase.toFixed(8)))
  console.log(coin[1]+' now: '+chalk.blueBright.bold(usd.toFixed(pairDecimals))+'      '+coin[0]+' now: '+chalk.blueBright.bold(btc.toFixed(8)))
  console.log(highest.toFixed(pairDecimals) + '    highest    '+chalk.yellow(calculatePercentage(highest, price).toFixed(2)) + '% of price')
  const priceColor = price.isLessThan(lastPrice)?chalk.red:chalk.green
  console.log(priceColor(price.toFixed(pairDecimals)) + '    price')
  console.log(chalk.blueBright(lastTrasnactionPrice.toFixed(pairDecimals))+'    last transaction price   '+ chalk.blue(calculatePercentage(price, lastTrasnactionPrice).toFixed(2))+"% of price")
  console.log(lowest.toFixed(pairDecimals) + '    lowest     '+chalk.yellow(calculatePercentage(lowest, price).toFixed(2)) + '% of price')

  console.log('my value in usd: ' + chalk.blue(usd.plus(btc.multipliedBy(price)).toFixed(pairDecimals)) + "$ no of transactions: " + chalk.blue(noOfTransactions) + "   tax collected: " + chalk.red(taxCollected.toFixed(1)) + '$ profit: ' + chalk.green(usd.plus(btc.multipliedBy(price)).minus(baseValue).toFixed(1)) + '$' + '$ profit in starter price: ' + chalk.green(usd.plus(btc.multipliedBy(startingPrice)).minus(baseValue).toFixed(1)) + '$')
  ...
  console.log('change was ' + color(changeFromLastTransaction.toFixed(2)+"%") + " = ",  color(price.minus(lastTrasnactionPrice).toFixed(pairDecimals)))

  if (buy) {
    const diffToLowestPrice = calculatePercentage(price, lowest)
    console.log(chalk.bgGreen('BUY:')+' change to lowest price ' + chalk.blue(diffToLowestPrice.toFixed(2))+"%" + "will buy at >= "+chalk.blue(minChangeToConsiderChangeOfTrend.toFixed(2)))
    const notProfitable = calculatePercentage(price, lastTrasnactionPrice).isGreaterThan(-0.3)
    if( notProfitable ){ buy=false
  ...
  if (sell) {
    ...
    if( notProfitable ){ sell=false
      console.log(chalk.bgRed('selling not profitable anymore'))
      }
    console.log(chalk.bgRed('SELL:')+'  change to highest price ' +  chalk.blue(diffToHighestPrice.toFixed(2)) + "% " + "will sell at <= "+chalk.blue(minChangeToConsiderChangeOfTrend.multipliedBy(-1).toFixed(2)))
  ...
  if ( !buy && !sell && changeFromLastTransaction.isLessThan(minChange.multipliedBy(-1) )) {
    console.log('will try to buy '+chalk.red(coin[0])+'. price was ' + lastTrasnactionPrice + ' and now is ' + price)
    resetCounters()
    buy = true
  }
  // should we consider selling ? 
  if (!buy && !sell && calculatePercentage(price, lastTrasnactionPrice).isGreaterThan(minChange)) {
    console.log('will try to sell '+chalk.red(coin[0])+'. price was ' + lastTrasnactionPrice + ' and now is ' + price)
    resetCounters()
    sell = true
  }
